...
async function detail(id) {
    try {
        ...
        const data = JSON.parse(base64Decode(dataStr.substring(0,dataStr.length-2).substring(6))).links;
        console.log('data', data);
        let playUrls = [];
        data.forEach(item => {
            let name = item['name'];
            let playUrl = item['url'];
            playUrls.push(name + '$' + encodeURIComponent(playUrl)); 
        });
        const video = {
            vod_id: id,
...
async function getPlayObj(id) {
    ...
    if(id.indexOf('sportsteam363') > 0) {
        let playUrl = await parseSportSteam363(id);
        let parse = 0;
        if (!playUrl.startsWith('http')) {
            playUrl = id;
            parse = 1;
        ...
        return {
            ...
            header: {
                'User-Agent': headers['User-Agent'],
                'Referer': 'http://play.sportsteam363.com/',
                'Accept': '*/*'
            }
        }
    }
...
async function parseSportSteam1234(url) {
    let key = 'toStringtoString';
        let iv = '2202030405060719';
    if (url.indexOf('gm.php') > 0 || url.indexOf('kbm.php') > 0) {
        ...
        for(const n of $('script')) {
            ...
            if(text.indexOf('encryptedBase64Str') > 0) {
                ...
                return decodeURIComponent(aesDecode(str, key, iv));
            }
        }
    }  
    return '';
...
async function parseSportSteam363(url) {
    if (url.indexOf('gm.php') > 0 || url.indexOf('kbm.php') > 0) {
        url = url.replace('gm.php', 'kbm.php');
    }
    const html = await get(url);
    ...
    for(const n of $('script')) {
        ...
        if(text.indexOf('encryptedBase64Str') > 0) {
            let str = text.split("var encryptedBase64Str = '")[1].split("';")[0];
            return 'http:' + decodeURIComponent(aesDecode(str, key, iv));
        }
    }
    return '';
}
